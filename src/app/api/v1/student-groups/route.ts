...
async function getHandler(req: NextRequest) {
  try {
    ...
    const filters = studentGroupFilterSchema.parse(
      Object.fromEntries(searchParams)
    );

    const { data, total } = await studentGroupService.list(
      session?.user ?? null,
      filters,
      pagination
    );
...
async function postHandler(req: NextRequest) {
  try {
    ...
    const validated = createStudentGroupSchema.parse(body);
    const group = await studentGroupService.create(
      session?.user ?? null,
      validated
    );
