...
async function getHandler(
  ...
  try {
    ...
    const { data, total } = await faultService.getMaintenanceLogs(
      session?.user ?? null,
      machineId,
      pagination
    );
...
async function postHandler(
  ...
  try {
    ...
    const validatedData = createMaintenanceLogSchema.parse({ ...body, machineId });

    const log = await faultService.createMaintenanceLog(session?.user ?? null, validatedData);
