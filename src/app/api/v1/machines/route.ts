...
import { createMachineSchema, machineFilterSchema } from "@/lib/validators";
...
async function getHandler(req: NextRequest) {
  try {
    ...
    const filters = machineFilterSchema.parse(Object.fromEntries(searchParams));

    const { data, total } = await machineService.getMachines(
      session?.user ?? null,
      filters,
      pagination
    );
...
async function postHandler(req: NextRequest) {
  try {
    ...
    return createdResponse(machine);
  } catch (error) {
    return handleApiError(error);
  }
}
